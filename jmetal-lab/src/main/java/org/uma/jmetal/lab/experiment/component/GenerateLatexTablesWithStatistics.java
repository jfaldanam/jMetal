...
/**
 * This class computes a number of statistical values (mean, median, standard deviation,
 * interquartile range) from the indicator files generated after executing {@link ExecuteAlgorithms}
 * and {@link ComputeQualityIndicators}. After reading the data files and calculating the values, a
 * Latex file is created containing an script that generates tables with the best and second best
 * values per indicator. The name of the file is {@link Experiment #getExperimentName()}.tex, which
 * is located by default in the directory {@link Experiment #getExperimentBaseDirectory()}/latex
 *
 * <p>Although the maximum, minimum, and total number of items are also computed, no tables are
 * generated with them (this is a pending work).
 *
...
public class GenerateLatexTablesWithStatistics implements ExperimentComponent {
  ...
  private List<List<List<List<Double>>>> readDataFromFiles() throws IOException {
    List<List<List<List<Double>>>> data =
        new ArrayList<List<List<List<Double>>>>(experiment.getIndicatorList().size());
      ...
      for (int problem = 0; problem < problemListSize; problem++) {
        ...
        for (int algorithm = 0; algorithm < algorithmListSize; algorithm++) {
          ...
          numberOfValues[indicator][problem][algorithm] =
              statValues.get("numberOfElements").intValue();
        }
      }
    }
  }
  ...
  private void generateLatexScript(List<List<List<List<Double>>>> data) throws IOException {
    String latexDirectoryName =
        experiment.getExperimentBaseDirectory() + "/" + DEFAULT_LATEX_DIRECTORY;
    File latexOutput;
  ...
  void printHeaderLatexCommands(String fileName) throws IOException {
    try (FileWriter os = new FileWriter(fileName, false)) {
      ...
      os.write("\\usepackage[margin=0.2in]{geometry}" + "\n");
  ...
  private void printData(
    ...
    try (FileWriter os = new FileWriter(latexFile, true)) {
      ...
      os.write(
          "\\caption{"
              + experiment.getIndicatorList().get(indicatorIndex).getName()
              + ". "
              + caption
              + "}"
              + "\n");

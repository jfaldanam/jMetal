...
import org.uma.jmetal.algorithm.multiobjective.smpso.SMPSOv2;
...
public class SMPSOStudy {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Experiment<DoubleSolution, List<DoubleSolution>> experiment =
            new ExperimentBuilder<DoubleSolution, List<DoubleSolution>>("SMPSOStudy")
  ...
  static List<ExperimentAlgorithm<DoubleSolution, List<DoubleSolution>>> configureAlgorithmList(
    ...
    for (int run = 0; run < INDEPENDENT_RUNS; run++) {
      for (int i = 0; i < problemList.size(); i++) {
        Algorithm<List<DoubleSolution>> algorithm = new SMPSO((DoubleProblem) problemList.get(i).getProblem(), 100,
                new CrowdingDistanceArchive<>(100),
                new PolynomialMutation(1.0 / problemList.get(i).getProblem().getNumberOfVariables(),
                20.0), new SequentialEvaluation<>(),
                new TerminationByEvaluations(25000)) ;
        algorithms.add(new ExperimentAlgorithm<>(algorithm, problemList.get(i), run));
      }
      for (int i = 0; i < problemList.size(); i++) {
        Algorithm<List<DoubleSolution>> algorithm = new SMPSOv2((DoubleProblem) problemList.get(i).getProblem(), 100,
                new CrowdingDistanceArchive<>(100),
                new PolynomialMutation(1.0 / problemList.get(i).getProblem().getNumberOfVariables(),
                        20.0), new SequentialEvaluation<>(),
                new TerminationByEvaluations(25000)) ;
